...
$R(p)$ is a non-decreasing function between 0 and 1, which takes value 1 for any $0 \leq p \leq 1$ when income (or another economic variable with poisitive support) is equally distributed among the individuals.
...
{I}\left(\frac{\widehat{Q}(p_n)}{\widehat{Q}(p_d)}\right)_{k} = \frac{\left(
  ...
  -\left( \frac{p_d - \mathbf{1}(y_i \leq \widehat{Q}(p_d))}{\widehat{f}(\widehat{Q}(p_d)) \, \widehat{N}}\right)\widehat{Q}(p_n)}{
  \widehat{Q}(p_d)^2}
$$
...
if_qsr_vals <- if_share_ratio(y_ordered, w_ordered, type = 4)
if_palma_vals <- if_share_ratio(y_ordered, w_ordered, type = 4, prob_numerator = 0.90, prob_denominator = 0.40)
if_q50_vals <- if_quantile(y_ordered, w_ordered, probs = 0.5, type = 6)
if_ratioquantiles_vals <- if_ratio_quantiles(y_ordered, w_ordered, prob_numerator = 0.90,
