...
#'
#' Computes the influence function of the ratio between two quantiles
#' (e.g., P90/P10) for all observations in the sample.
#' See \insertCite{deville1999variance;textual}{inequantiles} and
#' \insertCite{osier2009variance;textual}{inequantiles} for the definition of
#' influence functions in finite population theory.
#'
...
#'   denominator (default: \code{0.10}).
#' @param na.rm Logical; remove missing values before computing? Default: \code{TRUE}.
#'
#' @returns A numeric vector of influence function values, one per observation.
#'
#' @details
#' The influence function for the ratio \eqn{\widehat{R} = \widehat{Q}(p_n) / \widehat{Q}(p_d)}
...
#'     computed via \code{\link{csquantile}},
#'   \item \eqn{p_n} and \eqn{p_d} are the orders of the numerator and denominator
#'     quantiles, respectively,
#'   \item \eqn{\widehat{f}(\cdot)} is the estimated density function,
#'   \item \eqn{\widehat{N} = \sum_i w_i} is the estimated population size.
#' }
#'
#' The density \eqn{\widehat{f}(y)} is estimated via a Gaussian kernel:
#' \deqn{
#'   \widehat{f}(y) =
...
#' }
#' with bandwidth \eqn{h = 0.79 \cdot \mathrm{IQR} \cdot \widehat{N}^{-1/5}}.
#'
#' @examples
#' # On synthetic data
#' set.seed(1)
...
#'
#' @export
if_ratio_quantiles <- function(y, weights = NULL, type = 6,
                               prob_numerator   = 0.90,
  ...
  # Quantiles (needed for delta method denominator)
  Q_n <- unname(csquantile(y, weights = w, probs = prob_numerator,
                            type = type, na.rm = FALSE))
  Q_d <- unname(csquantile(y, weights = w, probs = prob_denominator,
                            type = type, na.rm = FALSE))

  # Influence functions for each quantile — delegate to if_quantile so that
  # bandwidth and density are computed identically (guarantees delta-method
  # consistency when the two are combined below).
  IF_Qn <- if_quantile(y, weights = weights, probs = prob_numerator,
                        type = type, na.rm = FALSE)
  IF_Qd <- if_quantile(y, weights = weights, probs = prob_denominator,
                        type = type, na.rm = FALSE)

  # Delta method: IF of ratio Q_n / Q_d
  (IF_Qn * Q_d - IF_Qd * Q_n) / Q_d^2
}
